...
#  if [ "$(id -u)" -eq 0 ]; then
#    if [ -n "$SUDO_USER" ]; then
#        print0 "This script has to run as root (not sudo)\n"; exit 1
#    fi
#  else
#    print0 "This script has to run as root\n"; exit 1
#  fi
...
function exists_columnstore_tmp_files_wrong_owner() {
  TMP_DIR=/tmp/columnstore_tmp_files
  if [ -d $TMP_DIR ]; then
...
function exists_readtomagic_errors() {
   ...
   if [ ! $READTOMAGIC == "0" ]; then
     echo "Errors found in crit logs: readToMagic." >> $WARNFILE
   fi
}
...
function exists_got_signal_errors() {
   set_log_error; if [ ! -f $LOG_ERROR ]; then return; fi
...
function report_columnstore_mounts() {
  ...
  COLSTORE_MOUNTS=$(mount | grep -i columnstore)
  if [ ! -z "$COLSTORE_MOUNTS" ]; then print0 "$COLSTORE_MOUNTS\n"; else print0 "No file system mount with directory name like columnstore.\n"; fi
  ech0
}
...
function report_slave_status(){
  ...
  SLAVE_STATUS=$(mariadb -Ae "show all slaves status\G"| grep Slave_SQL_Running_State | xargs)
...
function report_brm_saves_current() {
  ...
  BRMSAV=$(cat /var/lib/columnstore/data1/systemFiles/dbrm/BRM_saves_current || ech0 'BRM_saves_current does not exist.')
  print0 "$BRMSAV\n"
  ech0
}
...
if [ $CS_RUNNING ]; then
  ...
  cd /var/lib/columnstore/data1/systemFiles
...
touch $OUTPUTFILE; truncate -s0 $OUTPUTFILE;
...
\$fl = '/var/lib/columnstore/data'.\$dbroot.'/'.\$dir1.'.dir/'.\$dir2.'.dir/'.\$dir3.'.dir/'.\$dir4.'.dir/'.\$dir5.'.dir/FILE'.\$dir6.'.cdf';
print \$fl,' ',\$oid,\"\n\";"
...
if [ ! $USECACHED ]; then
  ...
  find /var/lib/columnstore -type f -name "FILE[0-9][0-9][0-9].cdf" >> $FILE_ALL_FILES
...
if [ $MISSING_COUNT == "1" ]; then
  ...
  print_color "\n### Summary ###\n"
  EM_COUNT=$(wc -l < $FILE_EM_FILES); ALL_FILE_COUNT=$(wc -l < $FILE_ALL_FILES);
